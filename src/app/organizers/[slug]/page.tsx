...
import { OrganizerDashboardButton } from "@/components/auth/OrganizerDashboardButton";
...
export default async function OrganizerPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [organizer, competitions] = await Promise.all([
    getOrganizerBySlug(slug).catch(() => undefined),
    getCompetitionsByOrganizer(slug).catch(() => []),
  ]);
    ...
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1">
          ...
          <OrganizerDashboardButton slug={slug} />
          </div>
